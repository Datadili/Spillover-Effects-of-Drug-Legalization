...
nocite: |
  @*
author:
  - name: \textbf{Dili Maduabum}\footnote{Yale University. I thank Sylvia Klosin, Anna Stansbury, and Brittany Street for their useful comments and suggestions.}
    firstname: Dili
    surname: Maduabum
...
sake of brevity, I use the term "states with any marijuana law" (AML) or
...
$$Y^j_{it} = \delta\mathbbm{1}{(Border * AML_{it})} + \alpha_i + \gamma_t + \lambda_i * t + \epsilon_{it}$$ {#eq-1}
...
controls for county-specific linear time trends whereas $\epsilon_{it}$ is a stochastic error term. The $\mathbbm{1}{(Border * AML_{it})}$ term is an indicator
...
$$Y^j_{it} = \sum^{k = 5}_{k=-5; k \neq -1}\theta_k\mathbbm{1}{(Border * AML_{i,t+k})} + \alpha_i + \gamma_t + \lambda_i * t + \epsilon_{it}$$ {#eq-event}


In this model, $\mathbbm{1}{(Border * AML_{i,t+k})}$ is a series of lags and leads for the years before
...
Table 1 shows the results of a difference-in-differences estimate of
...
\begin{table}[h]
\begin{center}
\textbf{Table 1}: The Direct and Indirect Effects of Bordering an AML State\\
\end{center}
\onehalfspacing
...
![DUIs](images/event_dui.png){#fig-dui width="60%"}
